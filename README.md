...
> **📢 本项目基于 [@callie0313/dylan-heartbeat](https://github.com/callie0313/dylan-heartbeat) 的生产实践改编**  
> 
> **核心创新**：摘要生命周期管理（原项目未包含）  
> **额外增加**：模块化活动系统 + 7 篇完整文档 + 跨平台支持 + 开箱即用示例

AI 伴侣自动唤醒系统 + 后台活动摘要注入

让 AI 伴侣在用户沉默时自主决策：是玩游戏、逛论坛、刷社交媒体，还是给用户发推送？后台活动自动拼成摘要注入对话，5 轮后停止（已进入记忆）。

**适用场景**：移动端无法接收服务器推送的 AI 伴侣 / 虚拟助手 App
...
Dylan Heartbeat 包含一个精美的 Web 管理面板，用于可视化监控和控制 AI 伴侣的后台活动。

![Admin Panel](admin/screenshot.png)

**特性**：
- 🎨 **毛玻璃拟态风格**：柔和的粉色调配色，半透明磨砂效果
- 🎯 **四屏切换**：开关控制 / AI 记录 / 审核日志 / 浏览器审批
- 🔄 **实时状态**：查看唤醒端运行状态、活动记录
- ⚙️ **在线配置**：切换模型、调整参数、管理预设方案
- 📊 **活动监控**：查看推送、游戏、论坛、日记等记录
- 📱 **响应式设计**：适配移动端和桌面端

查看 [admin/README.md](admin/README.md) 了解如何集成和使用。
